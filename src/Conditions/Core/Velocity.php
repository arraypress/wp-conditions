	...
	public static function get_all(): array {
		...
		return [
			...
			'velocity_orders_by_ip'             => [
				...
				'description'   => __( 'How many orders this IP has placed in the chosen time window. Catches burst attacks where one IP rapidly submits multiple orders. Suggested thresholds: ≥3 in the last hour for review, ≥5 in the last hour to block. Counts every order on the EDD orders table — completed, pending, refunded.', 'arraypress' ),
			...
			'velocity_orders_by_email'          => [
				...
				'step'          => 1,
				'units'         => Periods::get_window_units(),
				'description'   => __( 'How many orders this email has placed in the chosen time window. Less sensitive than by-IP because legitimate customers do place repeat orders, but high values (e.g. ≥5 in the last day) suggest scripted abuse.', 'arraypress' ),
			...
			'velocity_distinct_emails_by_ip'    => [
				...
				'step'          => 1,
				'units'         => Periods::get_window_units(),
				'description'   => __( 'How many DIFFERENT email addresses have ordered from this IP in the chosen time window. The strongest velocity signal — one machine cycling through multiple emails is the classic fraud-ring fingerprint. Suggested threshold: ≥3 distinct emails from one IP in the last day = block.', 'arraypress' ),
			...
			'velocity_distinct_ips_by_email'    => [
				...
				'step'          => 1,
				'units'         => Periods::get_window_units(),
				'description'   => __( 'How many DIFFERENT IPs have used this email in the chosen time window. High values suggest credential sharing or a stolen account being passed around. Less sensitive than the IP→emails direction because legitimate users do change networks (mobile/wifi/work). Suggested threshold: ≥5 in the last day.', 'arraypress' ),
			...
			'velocity_failed_orders_by_ip'      => [
				...
				'step'          => 1,
				'units'         => Periods::get_window_units(),
				'description'   => __( 'How many orders from this IP have FAILED in the chosen time window (failed / revoked / abandoned status). The classic card-testing signature: a fraudster trying stolen cards in rapid succession until one works. Suggested threshold: ≥3 in the last 5 minutes = block immediately.', 'arraypress' ),
			...
			'velocity_failed_orders_by_email'   => [
				...
				'step'          => 1,
				'units'         => Periods::get_window_units(),
				'description'   => __( 'How many orders for this email have FAILED in the chosen time window. Catches the same card-testing pattern as the IP version, useful when fraudsters rotate IPs but reuse the email.', 'arraypress' ),
