	...
	public static function get_all(): array {
		...
		return [
			...
			'velocity_orders_by_ip'             => [
				...
				'description'   => __( 'How many orders this IP has placed in the chosen time window. Catches burst attacks where one IP rapidly submits multiple orders. Suggested thresholds: ≥3 in 60 minutes for review, ≥5 in 60 minutes to block. Counts every order on the EDD orders table — completed, pending, refunded.', 'arraypress' ),
			...
			'velocity_orders_by_email'          => [
				...
				'step'          => 1,
				'units'         => Periods::get_units(),
				'description'   => __( 'How many orders this email has placed in the chosen time window. Less sensitive than by-IP because legitimate customers do place repeat orders, but high values (e.g. ≥5 in 24 hours) suggest scripted abuse.', 'arraypress' ),
			...
			'velocity_distinct_emails_by_ip'    => [
				...
				'step'          => 1,
				'units'         => Periods::get_units(),
				'description'   => __( 'How many DIFFERENT email addresses have ordered from this IP in the chosen time window. The strongest velocity signal — one machine cycling through multiple emails is the classic fraud-ring fingerprint. Suggested threshold: ≥3 distinct emails from one IP in 24 hours = block.', 'arraypress' ),
			...
			'velocity_failed_orders_by_ip'      => [
				...
				'step'          => 1,
				'units'         => Periods::get_units(),
				'description'   => __( 'How many orders from this IP have FAILED in the chosen time window (failed / revoked / abandoned status). The classic card-testing signature: a fraudster trying stolen cards in rapid succession until one works. Suggested threshold: ≥3 in 5 minutes = block immediately.', 'arraypress' ),
